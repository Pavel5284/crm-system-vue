...
const onFileChange = (e: Event) => {
  ...
  if (file.size > 2 * 1024 * 1024) {
    errorRef.value = t('avatar.fileTooLarge')
    return
  }
  if (!file.type.startsWith('image/')) {
    errorRef.value = t('avatar.onlyImage')
    return
  }
  ...
  reader.onerror = () => { errorRef.value = t('avatar.readError') }
...
<template>
  <div class="flex flex-col items-center gap-3 shrink-0">
    <div class="relative">
      ...
      <label :class="['absolute -bottom-2 -right-2 h-8 w-8 rounded-full bg-primary text-primary-foreground grid place-items-center cursor-pointer shadow hover:opacity-90 transition-opacity', isSaving && 'opacity-50 pointer-events-none']" :title="t('avatar.upload')">
    ...
    <button v-if="preview" type="button" class="text-xs text-red-500 hover:underline disabled:opacity-50" :disabled="isSaving" @click="onRemove">{{ t('avatar.remove') }}</button>
